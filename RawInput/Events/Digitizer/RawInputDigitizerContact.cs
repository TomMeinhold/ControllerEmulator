namespace RawInput.Digitizer
{
    ...
    public class RawInputDigitizerContact
    {
        ...
        public RawInputDigitizerContact(IEnumerable<HidButtonState> buttonStates, IEnumerable<HidValueState> valueStates)
        {
            ...
            IsInverted = buttons.TryGetValue(UsageInvert, out bool invert) ? invert : (bool?)null;
            IsButtonDown = buttons.TryGetValue(UsageBarrel, out bool barrel) ? barrel : (bool?)null;
            ...
            Width = values.TryGetValue(UsageWidth, out HidValueState width) ? width.CurrentValue : (int?)null;
            Height = values.TryGetValue(UsageHeight, out HidValueState height) ? height.CurrentValue : (int?)null;
            Identifier = values.TryGetValue(UsageIdentifier, out HidValueState identifier) ? identifier.CurrentValue : (int?)null;
        }
