namespace RawInput.Native
{
    ...
    public static class CfgMgr32
    {
        ...
        {
            TryLocateDevNode(devicePath, flags, out DeviceInstanceHandle device).EnsureSuccess();
        ...
        {
            ConfigReturnValue result = CM_Locate_DevNode(out IntPtr devInst, devicePath, flags);
        ...
        {
            IntPtr devInst = DeviceInstanceHandle.GetRawValue(device);
            uint size = 0;

            ConfigReturnValue result = CM_Get_DevNode_Property(devInst, in propertyKey, out _, IntPtr.Zero, ref size, 0);
